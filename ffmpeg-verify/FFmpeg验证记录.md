...
# 安装nasm
apt-get install nasm  # 缺少会报错，见 logs-mlu/configure-mlu-v1.log

# 配置configure
cd "./build-mlu"  # 以下命令均在build-mlu目录下执行
"../src/FFmpeg/configure" \
  --assert-level=2 \
  --prefix="./install" \
  --samples="../fate-suite" \
  ...
  --extra-cflags='-O2' \
  --extra-ldflags='' \
  | tee "../logs-mlu/configure-mlu.log"

# 编译
make -j"$(nproc)" | tee "../logs-mlu/make-mlu.log"
...
# 执行fate测试
LD_LIBRARY_PATH=".:./libavdevice:./libavfilter:./libavformat:./libavcodec:./libswresample:./libswscale:./libavutil" \
make fate | tee "../logs-mlu/fate-mlu.log"

# 查看ffmpeg版本
LD_LIBRARY_PATH=".:./libavdevice:./libavfilter:./libavformat:./libavcodec:./libswresample:./libswscale:./libavutil" \
  "./ffmpeg" -version \
  | tee "../logs-mlu/ffmpeg-version.log"

# 查看ffprobe版本
LD_LIBRARY_PATH=".:./libavdevice:./libavfilter:./libavformat:./libavcodec:./libswresample:./libswscale:./libavutil" \
  "./ffprobe" -version \
  | tee "../logs-mlu/ffprobe-version.log"
```
...
- MLU
  - MLU configure：
    - `./ffmpeg-verify/logs-mlu/configure-mlu-v1.log` 缺少nasm配置失败
    - `./ffmpeg-verify/logs-mlu/configure-mlu.log`
  - MLU build：`./ffmpeg-verify/logs-mlu/make-mlu.log`
  - MLU FATE：`./ffmpeg-verify/logs-mlu/fate-mlu.log`
  - MLU version：`./ffmpeg-verify/logs-mlu/ffmpeg-version.log`
  - MLU probe：`./ffmpeg-verify/logs-mlu/ffprobe-version.log`


## 结论
- CPU, CUDA, MLU 路径都已完成构建和 FATE 验证
